...
function init() {
    ...
    const savedRows = localStorage.getItem('pl_rows_per_page');
    if (savedRows) {
...
function loadData() {
    ...
    fetch('../data/pl_data.json')
        ...
        .then(data => {
            ...
            console.log('排列三数据加载成功，共加载', allData.length, '条记录');
            updateDisplay();
        })
        .catch(error => {
...
function setupEventListeners() {
    rowsPerPageSelect.addEventListener('change', () => {
        currentPage = 1;
        localStorage.setItem('pl_rows_per_page', rowsPerPageSelect.value);
        updateDisplay();
    });
